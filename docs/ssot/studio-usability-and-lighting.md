...
## 2026-07-20 authored-light helper 遮挡修复

Edit 模式下的 Point/Spot helper 是编辑覆盖层，不是场景模型，也不表达真实光照结果。它必须在相机旋转后仍可见，
包括 helper 位于 opaque 或 transparent/BLEND 模型内部及其后方的情况；Run 模式仍移除 helper 与 pick proxy。

- 根因是 helper 只有 `depthTest=false`，但仍处于 opaque render list 且保留 `depthWrite=true`。透明模型在后续
  transparent render list 绘制时会覆盖 helper，因此禁用深度测试本身不能保证最后可见。
- Point/Spot helper 统一使用 `transparent=true`、`opacity=1`、`depthTest=false`、`depthWrite=false`，并采用既有
  authoring overlay 约定 `renderOrder=1000`。pick proxy 的不可见材质、实体映射和 Edit-only 生命周期不变。
- 修复只属于 Runtime authoring visualization；真实 Point/Spot Light、光照值、选择/拾取、SceneDocument、revision、
  history、autosave、JSON/ZIP、ProjectRecord、archive 和 save semantics 均不改变。
- 回归 oracle 同时覆盖对象合同与真实 WebGL 输出：Point/Spot unit test 固定 helper overlay 属性和 pick proxy
  属性；Chromium 将 M0 GLB 第一材质改为 `BLEND`，把洋红 Point helper 放进模型后旋转相机，并要求 Canvas 中仍有
  helper 像素。修复前同一用例为 `0` 个匹配像素，修复后通过，证据截图为
  `artifacts/e2e/light-helper-transparent-model-1440x900.png`。

### Delivery ledger

- Source: `main@ec7e0d7`; repair branch: `fix/light-helper-occlusion`.
- Symptom: rotating the camera could let a scene model cover the authored-light sphere helper; root cause was the
  opaque/transparent render-list ordering described above.
- Scope: `packages/runtime/src/viewer/authored-light-controller.ts`, its focused test,
  `tests/e2e/light-authoring.spec.ts`, this SSoT section and the linked dev-workbench task.
- Verification: red unit failed on `renderOrder=0` and red Chromium found `0` helper pixels; fixed focused unit `8/8`,
  full light-authoring Chromium/WebGL `3/3`, final targeted rerun `1/1` and the 1440x900 artifact contains `1013`
  matching helper pixels. Full root/workspace typecheck, E2E TypeScript, ESLint, production build, Prettier and diff check
  passed.
- Standard/P2 review: goal alignment, user-visible orbit flow, Runtime ownership, Edit/Run lifecycle, helper/proxy
  separation, data/save contracts, failure isolation and test honesty all pass. No SceneDocument or public API change exists;
  the current structure still points to the accepted Runtime authoring-overlay architecture.
- Commit/push: the coherent repair slice is committed locally on this branch; the exact SHA is in the linked
  dev-workbench closeout. It is not pushed. Downstream target is `main`; no cherry-pick or merge has been performed.
