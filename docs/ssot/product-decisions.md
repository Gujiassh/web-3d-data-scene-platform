...
## PD-015：SceneWeave 品牌与包名边界

**决策：** 产品和 GitHub 仓库使用 `SceneWeave` / `sceneweave`。现有 `@web3d/*` 包名继续作为
`0.1.0-rc.1` SDK 合同；SceneDocument、archive、IndexedDB、偏好键、fixture generator 字符串和资产哈希不随
品牌改名。

**理由：** SceneWeave 提供可识别的产品品牌，而保留合同敏感标识可以避免一次展示层改名破坏消费者、
本地项目、确定性产物和既有证据。品牌发现性必须通过字面品类描述和真实能力建立，而不是改动底层数据含义。
