...
> 状态：产品方向、非生产校准、Critical 复审与 SceneDocument 1.4 实施批准均已完成
>
> 校准日期：2026-07-18
>
> 当前生产合同：SceneDocument 1.3.0
...
- 200 热点完整 CPU frame work p95 `1.40ms`，零热点 p95 `0.40ms`，增量 `1.00ms`；
- 呈现 RAF interval p95 `16.80ms`，大于 `25ms` 的 dropped interval 为 0；
- GPU timer p95 `2.17ms`；
- projection/occlusion p95 `0.40ms`，DOM/marker update p95 `0.70ms`；
- marker picking p95 `0.10ms`，300/300 ID 正确；
- 真实 Playwright pointer event 到后续 Chrome Paint p95 `2.68ms`；330 组 mark/Paint correlation 持久化后，
  去掉 30 条 warmup 可独立重算得到 `2.681ms`；
...
实施批准，CHK032 已关闭。SceneDocument 1.3 在迁移实现并通过验收前仍是生产权威。
...
1. 按 Document/Persistence -> Runtime -> React/Studio -> Run -> production acceptance 顺序实施。
2. 若实现需要改变已批准的持久化含义、保存语义、archive container、ProjectRecord、anchor、action set 或数量
   策略，必须先重新取得明确批准。
